        ...
        for (PowerPlant plant : reps.powerPlantRepository.findOperationalPowerPlantsByOwner(producer, getCurrentTick())) {
            ...
            if (!plant.isWithinTechnicalLifetime(getCurrentTick() + prolongYearsOfDismantlng)) {
                logger.warn("tick,"
                        ...
                        + ",lifeTime,"
                        + (getCurrentTick() - (plant.getConstructionStartTime() + plant.getActualPermittime() + plant
                                .getActualLeadtime())));
                // logger.info("       Dismantling power plant because the technical life time has passed: "
                // + plant);
