...
public class DismantlePowerPlantOperationalLossRole extends AbstractRole<ElectricitySpotMarket> {
    ...
    public void act(ElectricitySpotMarket market) {
        if (getCurrentTick() > 0) {
            ...
            // for loop updates fixed operational costs +5% for old pp and
            // calculates profitability for each powerplant
            // sum of profits during backlooking years divided by investment
            // costs
            for (PowerPlant plant : reps.powerPlantRepository.findOperationalPowerPlantsInMarket(market,
                ...
                // After expected lifetime, every year +5% fixed operating costs
                ...
                // logger.warn(plant.getProfitability() + ", " + profitability +
                // ", " + totalInvestment);
            ...
            // For all pp, if too old (expected plus max extra years) -->
            // dismantle
            for (PowerPlant plant : reps.powerPlantRepository.findOperationalPowerPlantsInMarket(market,
                    getCurrentTick())) {
                ...
                if (plant.getOwner().equals(reps.targetInvestorRepository.findInvestorByMarket(market))) {
                    ...
                    if (plant.getActualLifetime() > (prolongYearsOfDismantlng)) {
                        // logger.warn("maximum life reached!, " +
                        // plant.getName() + ", " + plant.getActualLifetime()
                        // + ", " + prolongYearsOfDismantlng);
                        // logger.warn("Age dismantling!");
            ...
            for (PowerPlant plant : reps.powerPlantRepository
                ...
                if (plant.getProfitability() < 0) {
                    ...
                    for (Segment currentSegment : reps.segmentRepository.findAll()) {
                        ...
                        if (currentSegment.getSegmentID() != 1) {

                            double segmentPortion = currentSegment.getSegmentID();
                    ...
                    if ((totalProfit + plant.getProfitability()) < 0) {
                        ...
                        // logger.warn("Economic dismantling!");
    ...
    public double calculateMarginalFuelCost(PowerPlant powerPlant, long clearingTick) {
        ...
        for (SubstanceShareInFuelMix mix : powerPlant.getFuelMix()) {
            ...
            fc += amount * fuelPrice;
            if (getCurrentTick() < 5) {
                logger.info("Calculating marginal cost {} and found a fuel price which is {} per unit of fuel", fc,
                        fuelPrice);
            }
        }
